...
static enum diff intuit_diff_type(ThreadSafeProgress*);
static nametype best_name PARAMS ((char * const *, int const *));
static int prefix_components PARAMS ((char *, int));
static size_t pget_line(ThreadSafeProgress*, int, int, int);
static size_t get_line(ThreadSafeProgress*);
...
static void skip_to PARAMS ((ThreadSafeProgress* progress, file_offset, LINENUM));
...
void
open_patch_file (ThreadSafeProgress*, char const *filename)
{
...
bool
there_is_another_patch(ThreadSafeProgress* progress)
{
   ...
   if (verbosity == VERBOSE)
      say (progress, "Hmm...");
   diff_type = intuit_diff_type(progress);
   ...
   if (verbosity == VERBOSE)
      say (progress, "  %sooks like %s to me...\n",
   ...
   {
      if (p_indent)
         say (progress, "(Patch is indented %d space%s.)\n", p_indent, p_indent==1?"":"s");
      if (p_strip_trailing_cr)
         say (progress, "(Stripping trailing CRs from patch.)\n");
      if (! inname)
      {
         char numbuf[LINENUM_LENGTH_BOUND + 1];
         say (progress, "can't find file to patch at input line %s\n",
              format_linenum (numbuf, p_sline));
         say (progress, strippath == -1
              ? "Perhaps you should have used the -p or --strip option?\n"
              : "Perhaps you used the wrong -p or --strip option?\n");
      }
   }

   skip_to(progress, p_start,p_sline);
   while (!inname) {
       //!!if (force || batch)
      {
         say (progress, "No file to patch.  Skipping patch.\n");
         skip_rest_of_patch = TRUE;
         return TRUE;
      }
      /*
        !!
      ask_file("File to patch: ");
      inname = fetchname (progress, buf, 0, (time_t *) 0);
      if (inname)
      {
...
static enum diff
intuit_diff_type (ThreadSafeProgress* progress)
{
   ...
   for (;;) {
      ...
      chars_read = pget_line (progress, 0, 0, 0);
      ...
      if (!stars_last_line && strnEQ(s, "*** ", 4))
         name[OLD] = fetchname (progress, s+4, strippath, &p_timestamp[OLD]);
      else if (strnEQ(s, "+++ ", 4))
         /* Swap with NEW below.  */
         name[OLD] = fetchname (progress, s+4, strippath, &p_timestamp[OLD]);
      else if (strnEQ(s, "Index:", 6))
         name[INDEX] = fetchname (progress, s+6, strippath, (time_t *) 0);
      ...
      } else
      {
         ...
         if (strnEQ(t, "--- ", 4))
         {
            ...
            name[NEW] = fetchname (progress, t+4, strippath, &timestamp);
      ...
      if ((diff_type == NO_DIFF || diff_type == UNI_DIFF)
         ...
         {
            char numbuf[LINENUM_LENGTH_BOUND + 1];
            say (progress, "missing header for unified diff at line %s of patch\n",
                 format_linenum (numbuf, p_sline));
         }
         goto scan_exit;
      }
      ...
      if ((diff_type == NO_DIFF
...

         {
            ...
            p_input_line -= 2;
            if (another_hunk (progress, retval, 0)
         ...
         {
            char numbuf[LINENUM_LENGTH_BOUND + 1];
            say (progress, "missing header for context diff at line %s of patch\n",
                 format_linenum (numbuf, p_sline));
         }
         goto scan_exit;
      }
...
static void
skip_to (ThreadSafeProgress* progress, file_offset file_pos, LINENUM file_line)
{
   register FILE *i = pfp;
   ...
   if ((verbosity == VERBOSE || !inname) && p_base < file_pos) {
      ...
      say (progress, "The text leading up to this was:\n--------------------------\n");
...
int
another_hunk (ThreadSafeProgress* progress, enum diff difftype, int rev)
{
   register char *s;
   ...
   if (rev)            /* backwards patch? */
      if (!pch_swap())
         say (progress, "Not enough memory to swap next hunk!\n");
   if (debug & 2) {
      LINENUM i;
      char special;
...
static size_t
get_line (ThreadSafeProgress* progress)
{
   return pget_line (progress, p_indent, p_rfc934_nesting, p_strip_trailing_cr);
}
...
static size_t
pget_line (ThreadSafeProgress* progress, int indent, int rfc934_nesting, int strip_trailing_cr)
{
   ...
   for (;;)
   {
      ...
      {
         s *= 2;
         b = reinterpret_cast<char*>(realloc (b, s));
         if (!b)
         {
            if (!using_plan_a)
               memory_fatal ();
...
patch_ends_in_middle_of_line:
   if (ferror (fp))
      read_fatal ();
   say (progress, "patch unexpectedly ends in middle of line\n");
   return 0;
}
...
void
do_ed_script (ThreadSafeProgress* progress, FILE *ofp)
{
   std::string ed_program = GetTortoiseDirectory();
   EnsureTrailingDelimiter(ed_program);
   ed_program += "ed.exe";
   ...
   if (!skip_rest_of_patch) {
      ...
      sprintf (buf, "%s %s%s", ed_program.c_str(), verbosity == VERBOSE ? "" : "- ",
               TMPOUTNAME);
      fflush (stdout);
      ...
      if (this_line_is_command) {
         if (pipefp)
            if (! fwrite (buf, sizeof *buf, chars_read, pipefp))
               write_fatal ();
         if (*t != 'd' && *t != 's') {
            while ((chars_read = get_line (progress)) != 0) {
               if (pipefp)
                  if (! fwrite (buf, sizeof *buf, chars_read, pipefp))
                     write_fatal ();
