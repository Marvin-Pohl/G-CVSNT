...
/* $Id: inp.cpp,v 1.4 2008/01/03 22:02:44 bullestock Exp $ */
...
static bool plan_a(ThreadSafeProgress* progress, char const *);/* yield FALSE if memory runs out */
static void plan_b(ThreadSafeProgress* progress, char const *);
static void report_revision(ThreadSafeProgress*, int);
...
void
scan_input (ThreadSafeProgress* progress, char *filename)
{
   using_plan_a = ! (debug & 16) && plan_a (progress, filename);
   if (!using_plan_a)
      plan_b(progress, filename);
   ...
   if (verbosity != SILENT)
   {
      ...
      if (verbosity == VERBOSE)
         say (progress, "Patching file %s using Plan %s...\n",
              filename, using_plan_a ? "A" : "B");
      else
         say (progress, "patching file %s\n", filename);
   }
}
...
static void
report_revision (ThreadSafeProgress* progress, int found_revision)
{
   ...
   {
      if (verbosity == VERBOSE)
         say (progress, "Good.  This file appears to be the %s version.\n", revision);
   }
   else if (force)
   {
      if (verbosity != SILENT)
         say (progress, "Warning: this file doesn't appear to be the %s version -- patching anyway.\n",
              revision);
   }
   else if (batch)
   ...
   {
      if (!ask_yesno(false, "This file doesn't appear to be the %s version -- patch anyway?",
                     revision))
         fatal ("aborted");
   }
}
...
void
get_input_file (ThreadSafeProgress* progress, char const *filename, char const *outname)
{
   ...
   if (inerrno && !pch_says_nonexistent (reverse))
   {
      errno = inerrno;
...
static bool
plan_a (ThreadSafeProgress* progress, char const *filename)
{
   ...
   if (revision)
   {
      ...
      report_revision (progress, found_revision);
   }
...
static void
plan_b (ThreadSafeProgress* progress, char const *filename)
{
   register FILE *ifp;
   register int c;
   register size_t len;
