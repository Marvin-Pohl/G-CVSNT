...
"$Id: partime.cpp,v 1.2 2005/09/18 20:40:46 bullestock Exp $";
...
{
   { { 'y', 'e', 'a', 'r' }, RELATIVE_CONS (tm_year,  1) },
   { { 'm', 'o', 'n', 't' }, RELATIVE_CONS (tm_mon ,  1) },
   { { 'f', 'o', 'r', 't' }, RELATIVE_CONS (tm_mday, 14) },
   { { 'w', 'e', 'e', 'k' }, RELATIVE_CONS (tm_mday,  7) },
   { { 'd', 'a', 'y' },      RELATIVE_CONS (tm_mday,  1) },
   { { 'h', 'o', 'u', 'r' }, RELATIVE_CONS (tm_hour,  1) },
   { { 'm', 'i', 'n' },      RELATIVE_CONS (tm_min ,  1) },
   { { 's', 'e', 'c' },      RELATIVE_CONS (tm_sec ,  1) },
   {"", TM_UNDEFINED}
};
...
static struct name_val const zone_names[] =
{
   ...
   zs (  000, "gmt" ), /* Greenwich Mean, British Summer */
   zs (  000, "wet" ), /* Western European */
   zs (  100, "cet" ), /* Central European */
   zs (  100, "met" ), /* Middle European (bug in old tz versions) */
   zs (  100, "mez" ), /* Mittel-Europaeische Zeit */
   zs (  200, "eet" ), /* Eastern European */
   zs (  530, "ist" ),      /* India */
...
/* Parse an initial prefix of S of length DIGITS;
   ...
   Return the first character after the prefix, or 0 if it wasn't parsed.  */
static char const *
parse_ranged (char const *s,
              int digits, int lo, int hi, int *res)
{
...
/* Parse an initial prefix of S of length DIGITS;
   ...
   Return the first character after the prefix, or 0 if it wasn't parsed.  */
static char const *
parse_decimal (char const *s,
               int digits, int lo, int hi, int resolution, int *res, int *fres)
{
   s = parse_fixed (s, digits, res);
   if (s && lo <= *res && *res <= hi)
   {
...
/* Parse an initial prefix of S, matching the pattern whose code is C.
   ...
   Return the first character after the prefix, or 0 if it wasn't parsed.  */
static char const *
parse_pattern_letter (char const *s,
                      int c,
                      struct partime *t)
{
   char const *s0 = s;
...
static int
merge_partime (struct partime *t,
